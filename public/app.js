...
function renderDashboard() {
  ...
  if (hasDays || hasWeeks) {
    ...
    chartsHtml = '<div class="empty-state"><div class="empty-icon">⚡</div>' +
      ...
      '<button class="btn btn-outline" onclick="loadSampleData()">📦 Load Demo Data</button>' +
      '</div></div>';
  }
...
function renderLogToday(editDay) {
  ...
  document.getElementById('main').innerHTML =
    ...
    // WEEKLY INCOME (only if money pillar on and not editing)
    (!isEditing && isPillarOn('money') ? renderWeeklyIncomeInline() : '') +
...
async function loadSampleData() {
  ...
  const profile = { name: '', gymDaysPerWeek: 5, weeklyIncomeGoal: 1200, weeklyNetworkGoal: 3, weeklyReadGoal: 100,
    ...
    onboarded: true };
  const muscleGroups = ['chest', 'back', 'legs', 'shoulders', 'arms', 'full body'];
  const gymNotePool = ['Great session', 'Felt strong today', 'New PR on bench', 'Pushed through fatigue', 'Leg day — tough but done', 'Light day', ''];
  const foodNotePool = ['Ate clean all day', 'Hit protein goal', 'Skipped breakfast, not great', 'Meal prepped — easy day', 'Ate out but kept it reasonable', ''];
  const netNotePool  = ['Met an investor at the gym', 'Reconnected with old client', 'Cold-messaged 5 prospects on Instagram', 'Attended a networking event', 'Made new friend in my building', ''];
  const moneyPool    = ['Made 25 cold calls, set 2 appointments', 'Followed up with 10 warm leads', 'Closed a deal — great day', 'Spent day driving area and prospecting', 'Door knocked for 4 hours', 'Made 30 calls, no closes — keep pushing', 'Had 3 presentations today'];
  const weekNotePool = ['Closed 2 deals this week', 'Slow week — bad weather', 'Strong finish on Friday', 'Referral from last month came through', ''];
  const dayNotePool = [
    'Felt focused and energized today. Stayed off my phone in the morning and it made a huge difference.',
    'Tough day mentally but I showed up anyway. Proud I didn\'t skip the gym.',
    'Had a great conversation with a prospect — felt a real connection. Following up tomorrow.',
    'Low energy. Didn\'t sleep well and it showed. Need to get to bed earlier.',
    'Big win today! Closed a deal I\'ve been working for weeks. Hard work pays off.',
    'Slow day. Used the downtime to read and plan the week ahead.',
    'Felt scattered. Too many small tasks, not enough deep work. Tomorrow I block my morning.',
    ''
  ];
  const ideasList = [
    { title: 'Referral program with gym members', description: 'Ask gym contacts to refer potential clients — offer them $100 per closed deal', status: 'active' },
    { title: 'Weekend solar pop-up at Home Depot', description: 'Set up a table at Home Depot on Saturdays to talk to homeowners directly', status: 'exploring' },
    { title: 'Online sales training course', description: 'Record a Udemy course on commission sales techniques — passive income', status: 'exploring' },
    { title: 'Become a solar panel dealer', description: 'Buy leads wholesale and resell to other reps', status: 'exploring' },
    { title: 'Instagram personal brand', description: 'Document the sales/gym lifestyle to attract clients and partnerships', status: 'active' }
  ];

  const readSummaryPool = [
    'Learned that assets put money in your pocket and liabilities take money out. Most people buy liabilities thinking they\'re assets.',
    'The rich focus on building assets — businesses, investments, real estate — while the poor and middle class work for wages.',
    'Fear and greed drive most financial decisions. The rich learn to manage these emotions; others are controlled by them.',
    'Accounting is the language of money. Understanding a balance sheet and income statement is essential to building wealth.',
    'The rat race: work, earn, spend, repeat. Breaking out requires building assets that generate passive income.',
    'Corporations are the secret of the rich — they pay expenses before taxes, while employees pay taxes before expenses.',
    'Financial intelligence is the combination of accounting, investing, understanding markets, and knowing the law.',
    'Mind your own business: your job is your income, but your business is your asset column — what you build for yourself.',
    'Money is not real — it is an agreed-upon illusion. The person who understands this controls it; others chase it.',
    'The rich don\'t work for money — they have money work for them through cash-flowing investments.'
  ];
  const bookId = uid();
  ...
  for (let i = 59; i >= 0; i--) {
    ...
    // Gym: aim for 5x/week, skip Sundays randomly
    const gymDone = !isWeekend && Math.random() > 0.25;
    const restDay = dayOfWeek === 0;

    const foodRating = Math.round(2.5 + Math.random() * 2.5); // 2–5

    const netCount = Math.random() > 0.55 ? Math.floor(Math.random() * 3) + 1 : 0;

    days.push({
      id: uid(), date: dateStr,
      gym: { done: gymDone, muscleGroup: gymDone ? muscleGroups[i % muscleGroups.length] : '', duration: gymDone ? 45 + Math.round(Math.random() * 30) : 0, notes: gymDone ? gymNotePool[Math.floor(Math.random() * gymNotePool.length)] : '' },
      food: { rating: foodRating, notes: foodNotePool[Math.floor(Math.random() * foodNotePool.length)] },
      networking: { count: netCount, notes: netCount > 0 ? netNotePool[Math.floor(Math.random() * netNotePool.length)] : '' },
      money: { activities: !isWeekend ? moneyPool[Math.floor(Math.random() * moneyPool.length)] : '', income: 0 },
      reading: (() => {
        const read = Math.random() > 0.35;
        return {
          pages: read ? Math.floor(Math.random() * 45) + 5 : 0,
          bookId: read ? bookId : '',
          bookTitle: read ? 'Rich Dad Poor Dad' : '',
          summary: read ? readSummaryPool[Math.floor(Math.random() * readSummaryPool.length)] : ''
        };
      })(),
  ...
  // Give the most recent demo day an itemized food log to showcase the counter
  if (days.length) {
    const sampleFoods = [
      { f: 'Egg, whole', g: 100 }, { f: 'Oats, dry', g: 60 },
      { f: 'Chicken breast (cooked)', g: 200 }, { f: 'Rice, white (cooked)', g: 200 },
      { f: 'Banana', g: 118 }, { f: 'Greek yogurt, nonfat', g: 170 }
    ];
    const fl = sampleFoods.map(s => {
      const food = FOOD_DB.find(x => x.n === s.f); const m = foodMacros(food, s.g);
      return { id: uid(), name: food.n, grams: s.g, unit: 'g', qty: s.g, kcal: m.kcal, p: m.p, c: m.c, f: m.f };
    });
    const tot = foodLogTotals(fl);
    const last = days[days.length - 1];
    last.foodLog = fl;
    last.calories = Math.round(tot.kcal);
    last.eaten = { protein: Math.round(tot.p), carbs: Math.round(tot.c), fat: Math.round(tot.f) };
  }

  // Weekly income — last 12 weeks
  for (let i = 11; i >= 0; i--) {
    const monday = new Date(now);
    monday.setDate(monday.getDate() - monday.getDay() + 1 - i * 7);
    if (monday > now) continue;
    const base = 600 + Math.round(Math.random() * 700);
    const spike = Math.random() > 0.75;
    const slump = Math.random() > 0.8;
    weeks.push({
      id: uid(),
      weekStart: monday.toISOString().split('T')[0],
      income: Math.round((spike ? base * 1.7 : slump ? base * 0.45 : base) * 100) / 100,
      notes: weekNotePool[Math.floor(Math.random() * weekNotePool.length)]
    });
  }
  ...
  const books = [{
    ...
    totalPages: 336,
    startDate: (() => { const d = new Date(now); d.setDate(d.getDate() - 45); return d.toISOString().split('T')[0]; })(),
    status: 'reading',
    finishedDate: null
  }];

  const ideas = ideasList.map(i => ({ ...i, id: uid(), createdAt: todayStr(), notes: '' }));

  const contacts = [
    { id: uid(), name: 'Marcus Williams', role: 'Owns 3 Airbnbs in the area', met: 'Met at the gym, bench press area', category: 'prospect', phone: '760-555-0182', social: '@marcus.w', followUpDate: todayStr(), dealValue: 18000, notes: 'Mentioned his electricity bill is insane. Very interested in solar.', status: 'warm', addedDate: todayStr() },
    { id: uid(), name: 'Diana Reyes', role: 'Real estate agent, sells 15+ homes/year', met: 'Spin class, Tuesday morning', category: 'referral', phone: '760-555-0341', social: '@dianasells', followUpDate: '', dealValue: 0, notes: 'Can refer homebuyer clients who need solar. She has a big network.', status: 'contacted', addedDate: todayStr() },
    { id: uid(), name: 'Kevin Park', role: 'Business owner — dry cleaning chain', met: 'Networking event downtown', category: 'client', phone: '310-555-0229', social: '', followUpDate: '', dealValue: 22000, notes: 'Closed deal last month. Very happy. Ask for referrals.', status: 'closed', addedDate: todayStr() },
    { id: uid(), name: 'Tom Bradley', role: 'Contractor, builds new homes', met: 'At the gym, locker room', category: 'partner', phone: '760-555-0447', social: '@tombradleybuilds', followUpDate: '', dealValue: 0, notes: 'Could partner on new construction projects — include solar from day 1.', status: 'new', addedDate: todayStr() }
  ];
  // Sample weigh-ins — a gentle downward trend over the last ~8 weeks (stored in kg)
  const weights = [];
  for (let i = 8; i >= 0; i--) {
    const d = new Date(now); d.setDate(d.getDate() - i * 7);
    const kg = Math.round((84 - (8 - i) * 0.4 + (Math.random() * 0.6 - 0.3)) * 10) / 10;
