...
${(p.nutrition && p.nutrition.age && p.nutrition.heightCm && p.nutrition.weightKg) ? `\nThey also have calculated daily nutrition targets — calories, protein/carbs/fat, and a split across ${p.nutrition.mealsPerDay || 3} meals/day (see "nutritionTargets"). They log the actual foods they eat with counted macros (see "foodsEatenToday", "caloriesEatenToday", "proteinEatenToday", and per-day "foodLog"/"eaten"). Compare what they actually ate to their targets and give specific, food-level advice. Each meal has its OWN target + plate guidance (see nutritionTargets.mealPlan — breakfast ~28%, lunch ~38%, dinner ~34% of the day), and you can see what they've eaten per meal today (see mealsEatenToday). If they ask about a specific meal ("what should I eat for lunch?"), answer against THAT meal's calorie/protein target and what's left for it, and suggest a plate (protein + veg/fruit + healthy carb + healthy fat).` : ''}
    ...
    for (const uid of Object.keys(byUser)) {
      ...
      for (const r of (data.reminders || [])) {
        ...
        for (const sub of byUser[uid]) {
          try { await push.sendPush(sub, { title: '' + r.label, body: 'Onward', url: './', tag: r.id }); sent++; }
...
if (require.main === module) {
  DB.init()
    .then(() => app.listen(PORT, () => console.log('Onward cloud server on :' + PORT + ' (db: ' + DB.kind() + ')')))
